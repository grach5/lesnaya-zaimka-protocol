...
export default function ReserveForm({ locale = "ru" }: Props) {
  ...
  function handleSubmit(e: SubmitEvent<HTMLFormElement>) {
    ...
    if (date && date < new Date().toISOString().slice(0, 10)) nextErrors.date = t.ui.common.errDatePast;
    if (guests && (Number(guests) < 1 || !Number.isFinite(Number(guests)))) nextErrors.guests = t.ui.common.errGuests;
    ...
    // Текст сообщения всегда по-русски — читают сотрудники ресторана.
    const lines = [
